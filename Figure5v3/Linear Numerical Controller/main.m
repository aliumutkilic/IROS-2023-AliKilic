...
options1 = odeset('Events',@pausevent,'AbsTol',1e-8);        %Pauses upon reaching midstep
options2 = odeset('Events',@endevent,'AbsTol',1e-8);         %Pauses upon reaching toe-off
...
param1 = fmincon(@(param)fn_linear_objective(param,p),param0,[],[],[],[],lb,ub,@(param)fn_linear_constraint(param,p),options_optim);
k1 = param1(1) ;
k2 = param1(2);

else

param1 = fmincon(@(param)fn_linear_objective(param,p),param0,[],[],[],[],lb,ub,@(param)fn_linear_constraint1(param,p),options_optim);
k1 = param1(1) - gain1 * init_cond(4);
k2 = param1(2) + gain2 * (xdot_desired - init_cond(2));
